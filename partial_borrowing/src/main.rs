...
//where T: Copy,
{
    pub fn x_mut(&mut self) -> &'a mut T {
        &mut self.x
    }

    pub fn y_mut(&mut self) -> &'a mut T {
        &mut self.y
    }
}
...
fn partial_borrow_test() {
    ...
    let &mut mut x_tuple_mut = point_tuple.x_mut();
    let &mut mut y_tuple_mut = point_tuple.y_mut();
    ...
    *y_mut = 2;
    ...
    x_tuple_mut = 2;
    y_tuple_mut = 2;
    assert_eq!(point_tuple.0, 1);
    assert_eq!(point_tuple.1, 1);
}
